...
let all_time_chart, day_chart, recover_rate_chart

window.onload = async () => {
   console.log('ready....')

   // only init chart on pagr loaded first time
...
loadsummary = async (country) => {
    ...
    for (let i = 0; i < 10; i++) {
        let row = `
            <tr>
                <td>${casesByCountries[i]?.Country}</td>
                <td>${numberWithCommas(summary?.TotalConfirmed)}</td>
                <td>${numberWithCommas(summary?.TotalRecovered)}</td>
                <td>${numberWithCommas(summary?.TotalDeaths)}</td>
            </tr>
        `
        table_countries_body.innerHTML += row
    }
...
loadAllTimeChart = async(country) => {
    ...
    if(isGlobal(country)) {
        ...
        let confirmed =  await covidApi.renderWorldData(country,CASE_STATUS.confirmed)
        let recovered =  await covidApi.renderWorldData(country, CASE_STATUS.recovered)
        let deaths =  await covidApi.renderWorldData(country, CASE_STATUS.deaths)
